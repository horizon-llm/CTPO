...
def _iter_numeric_values(value: Any):
    """Yield numeric scalars from nested metric values."""
...
def reduce_metrics(metrics: dict[str, Union["Metric", list[Any]]]) -> dict[str, Any]:
    ...
    for key, val in metrics.items():
        if isinstance(val, Metric):
            ...
            numeric_vals = list(_iter_numeric_values(val))
            if len(numeric_vals) == 0:
                metrics[key] = 0.0
            elif "max" in key:
                metrics[key] = float(np.max(numeric_vals))
            elif "min" in key:
                metrics[key] = float(np.min(numeric_vals))
            else:
                metrics[key] = float(np.mean(numeric_vals))
    return metrics
