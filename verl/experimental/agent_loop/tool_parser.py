...
class HermesToolParser(ToolParser):
    ...
    def __init__(self, tokenizer) -> None:
        ...
        # Fallback patterns for weaker tool-format-following models (e.g. base checkpoints).
        self.json_tool_call_regex = regex.compile(
            r'\{\s*"name"\s*:\s*"(?P<name>[^"]+)"\s*,\s*"arguments"\s*:\s*(?P<arguments>\{.*?\})\s*\}',
            regex.DOTALL,
        )
        self.code_block_regex = regex.compile(r"```(?:py|python)?\n(.*?)\n```", regex.DOTALL)

    def _extract_fallback_calls(self, text: str) -> tuple[str, list[FunctionCall]]:
        function_calls: list[FunctionCall] = []
        content = text

        # 1) Parse raw JSON tool calls like:
        # {"name":"code_interpreter","arguments":{"code":"print(1)","timeout":30}}
        for match in self.json_tool_call_regex.finditer(text):
            try:
        ...
        # 2) SimpleTIR-style fallback: if model emits python code blocks, treat them as code_interpreter calls.
        # This preserves tool execution even when model fails to wrap with <tool_call>...</tool_call>.
        for match in self.code_block_regex.finditer(text):
            ...
            function_calls.append(
                FunctionCall(
                    ...
                    arguments=json.dumps({"code": code, "timeout": 30}, ensure_ascii=False),
                )
            )
        return content, function_calls
    ...
    async def extract_tool_calls(self, responses_ids: list[int]) -> tuple[str, list[FunctionCall]]:
        ...
        # NOTE: Disabled strict Hermes branch for now.
        # We intentionally rely on SimpleTIR-style fallback extraction to reduce
        # format-following burden for base models.
        #
        ...
        # Optional robust fallback (enabled by default) for non-standard outputs.
        if os.getenv("VERL_HERMES_FALLBACK", "1") == "1":
            return self._extract_fallback_calls(text)
        return content, function_calls
