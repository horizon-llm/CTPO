...
DEFAULT_TOOL_USE_SYSTEM_PROMPT = (
    ...
    "Code format:\n"
    "1) When you want to run code, output exactly one fenced code block.\n"
    "2) Use ```python ... ``` format.\n"
...
class AgentData:
    ...
    def __init__(
        ...
        self.sandbox_call_count = 0
        self.sandbox_success_count = 0
        self.sandbox_code_lines_total = 0
        self.sandbox_code_lines_success_total = 0
        self.has_boxed_answer = False
...
class ToolAgentLoop(AgentLoopBase):
    ...
    async def run(self, sampling_params: dict[str, Any], **kwargs) -> AgentLoopOutput:
        ...
        if not messages:
            messages = [{"role": "system", "content": DEFAULT_TOOL_USE_SYSTEM_PROMPT}, *messages]
        elif messages[0].get("role") == "system":
            existing_system = messages[0].get("content", "")
            if DEFAULT_TOOL_USE_SYSTEM_PROMPT not in existing_system:
                messages[0]["content"] = f"{DEFAULT_TOOL_USE_SYSTEM_PROMPT}\n\n{existing_system}"
        else:
            messages = [{"role": "system", "content": DEFAULT_TOOL_USE_SYSTEM_PROMPT}, *messages]
        ...
        # Response-level success: at least one successful sandbox call in this response trajectory.
        sandbox_success = int(agent_data.sandbox_success_count > 0)
        output.extra_fields.update(
            {
                "turn_scores": agent_data.turn_scores,
                "tool_rewards": agent_data.tool_rewards,
                "sandbox_called": sandbox_called,
                ...
                # SimpleTIR-style void turn mask:
                # keep(1) if sample used tool at least once OR produced a boxed answer, else mask(0).
                "void_turn_mask": float((agent_data.sandbox_call_count > 0) or agent_data.has_boxed_answer),
            }
        )
        return output
    ...
    @staticmethod
    def _extract_code_line_count(tool_call: FunctionCall) -> int:
        try:
        ...
        if m:
            code = m.group(1)
        # Count non-empty lines to avoid inflating by blank lines.
    ...
    async def _handle_generating_state(
        ...
        # Extract tool calls and keep text to detect whether model already produced boxed final answer.
