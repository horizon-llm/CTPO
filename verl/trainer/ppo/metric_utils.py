...
def _safe_tensor_stats(x: torch.Tensor) -> tuple[float, float, float]:
    """Return (mean, max, min) with empty-tensor safety."""
    if x.numel() == 0:
        return 0.0, 0.0, 0.0
    return torch.mean(x).detach().item(), torch.max(x).detach().item(), torch.min(x).detach().item()


def _safe_np_stats(x: np.ndarray) -> tuple[float, float, float]:
    """Return (mean, min, max) with empty-array safety."""
    if x.size == 0:
        return 0.0, 0.0, 0.0
...
def compute_data_metrics(batch: DataProto, use_critic: bool = True) -> dict[str, Any]:
    ...
    ret_mean, ret_max, ret_min = _safe_tensor_stats(valid_returns)

    if use_critic:
        values = batch.batch["values"]
        valid_values = torch.masked_select(values, response_mask)
        val_mean, val_max, val_min = _safe_tensor_stats(valid_values)
    ...
    # Sandbox/tool-use metrics (response-level)
    sandbox_call_count = None
    sandbox_success_count = None
    sandbox_called = None
    sandbox_success = None

    # A) per-response call count
    if "sandbox_call_count" in batch.non_tensor_batch:
        sandbox_call_count = np.asarray(batch.non_tensor_batch["sandbox_call_count"], dtype=np.float32)
        call_mean, call_min, call_max = _safe_np_stats(sandbox_call_count)
        metrics["sandbox/call_count/mean"] = call_mean
        metrics["sandbox/call_count/min"] = call_min
        metrics["sandbox/call_count/max"] = call_max

    # B) per-response successful call count
    if "sandbox_success_count" in batch.non_tensor_batch:
        sandbox_success_count = np.asarray(batch.non_tensor_batch["sandbox_success_count"], dtype=np.float32)
        succ_mean, succ_min, succ_max = _safe_np_stats(sandbox_success_count)
        metrics["sandbox/success_count/mean"] = succ_mean
        metrics["sandbox/success_count/min"] = succ_min
        metrics["sandbox/success_count/max"] = succ_max

    # C) response-level call/success rates
    if "sandbox_called" in batch.non_tensor_batch:
        sandbox_called = np.asarray(batch.non_tensor_batch["sandbox_called"], dtype=np.float32)
        metrics["sandbox/called/rate"] = float(np.mean(sandbox_called)) if sandbox_called.size > 0 else 0.0

    if "sandbox_success" in batch.non_tensor_batch:
        sandbox_success = np.asarray(batch.non_tensor_batch["sandbox_success"], dtype=np.float32)
        metrics["sandbox/success/rate"] = float(np.mean(sandbox_success)) if sandbox_success.size > 0 else 0.0
        if sandbox_called is not None:
            called_count = float(np.sum(sandbox_called))
            success_count = float(np.sum(sandbox_success))
            metrics["sandbox/success/among_called_rate"] = success_count / called_count if called_count > 0 else 0.0

    if sandbox_call_count is not None and sandbox_success_count is not None:
        total_calls = float(np.sum(sandbox_call_count))
        total_success = float(np.sum(sandbox_success_count))
        metrics["sandbox/success_count/per_call_rate"] = total_success / total_calls if total_calls > 0 else 0.0

    # D) code-line aggregates
    if "sandbox_code_lines_total" in batch.non_tensor_batch:
        sandbox_code_lines_total = np.asarray(batch.non_tensor_batch["sandbox_code_lines_total"], dtype=np.float32)
        code_mean, code_min, code_max = _safe_np_stats(sandbox_code_lines_total)
        metrics["sandbox/code_lines/mean"] = code_mean
        metrics["sandbox/code_lines/min"] = code_min
        metrics["sandbox/code_lines/max"] = code_max

    if "sandbox_code_lines_success_total" in batch.non_tensor_batch:
        sandbox_code_lines_success_total = np.asarray(
            batch.non_tensor_batch["sandbox_code_lines_success_total"], dtype=np.float32
        )
        code_succ_mean, code_succ_min, code_succ_max = _safe_np_stats(sandbox_code_lines_success_total)
        metrics["sandbox/code_lines_success/mean"] = code_succ_mean
        metrics["sandbox/code_lines_success/min"] = code_succ_min
        metrics["sandbox/code_lines_success/max"] = code_succ_max
