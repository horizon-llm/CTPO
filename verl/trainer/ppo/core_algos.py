...
def agg_loss(
    ...
    if loss_agg_mode == "token-mean":
        ...
        if torch.is_tensor(batch_num_tokens):
            denom = batch_num_tokens.clamp(min=1)
        else:
            denom = max(batch_num_tokens, 1)
        ...
        if torch.is_tensor(global_batch_size):
            denom = global_batch_size.clamp(min=1)
        else:
            denom = max(global_batch_size, 1)
...
def _compute_policy_loss_cum_token_impl(
    old_log_prob: torch.Tensor,
    log_prob: torch.Tensor,
    advantages: torch.Tensor,
    response_mask: torch.Tensor,
    ...
    config: Optional[ActorConfig] = None,
    rollout_is_weights: torch.Tensor | None = None,
    ratio_mode: str = "geomean",
) -> tuple[torch.Tensor, dict[str, Any]]:
    """
    Compute PPO objective using cumulative-token importance ratio.

    Compute CTPO loss with configurable cumulative ratio mode.
    - geomean: exp(mean_{<=t}(log π - log π_old))
    - cumprod: exp(sum_{<=t}(log π - log π_old))
    """
    ...
    mask = response_mask.to(dtype=negative_approx_kl.dtype)
    cumulative_sum = torch.cumsum(negative_approx_kl * mask, dim=-1)
    if ratio_mode == "geomean":
    ...
    elif ratio_mode == "cumprod":
        kl_values = torch.where(mask > 0, cumulative_sum, torch.zeros_like(cumulative_sum))
    else:
        raise ValueError(f"Unsupported cum-token ratio mode: {ratio_mode}")

    # Stop-gradient decomposition used by sequence/cum-token style losses.
    ...
    # Importance-weight and clip trigger diagnostics (SimpleTIR-style).
    valid_mask = response_mask > 0
    valid_count = int(valid_mask.sum().item())
    ratio_valid = ratio[valid_mask]
    adv_valid = advantages[valid_mask]

    # PPO-clip effective trigger conditions by advantage sign.
    clip_high_mask = (ratio > (1.0 + clip_ratio_high)) & (advantages > 0) & valid_mask
    clip_low_mask = (ratio < (1.0 - clip_ratio_low)) & (advantages < 0) & valid_mask
    dual_clip_mask = torch.gt(clip_pg_losses1, pg_losses3) & (advantages < 0) & valid_mask
    clip_any_mask = clip_high_mask | clip_low_mask | dual_clip_mask

    clip_high_count = int(clip_high_mask.sum().item())
    clip_low_count = int(clip_low_mask.sum().item())
    dual_clip_count = int(dual_clip_mask.sum().item())
    clip_total_count = int(clip_any_mask.sum().item())

    pg_metrics = {
        "actor/pg_clipfrac": pg_clipfrac.detach().item(),
        "actor/ppo_kl": ppo_kl.detach().item(),
        "actor/pg_clipfrac_lower": pg_clipfrac_lower.detach().item(),
        "pg_update/is_ratio/mean": ratio_valid.mean().item() if valid_count > 0 else 0.0,
        "pg_update/is_ratio/min": ratio_valid.min().item() if valid_count > 0 else 0.0,
        "pg_update/is_ratio/max": ratio_valid.max().item() if valid_count > 0 else 0.0,
        "pg_update/is_ratio/p99": torch.quantile(ratio_valid, 0.99).item() if valid_count > 0 else 0.0,
        "pg_update/is_ratio/p95": torch.quantile(ratio_valid, 0.95).item() if valid_count > 0 else 0.0,
        "pg_update/is_ratio/p90": torch.quantile(ratio_valid, 0.90).item() if valid_count > 0 else 0.0,
        "pg_update/is_ratio/p75": torch.quantile(ratio_valid, 0.75).item() if valid_count > 0 else 0.0,
        "pg_update/is_ratio/p50": torch.quantile(ratio_valid, 0.50).item() if valid_count > 0 else 0.0,
        "pg_update/is_ratio/p25": torch.quantile(ratio_valid, 0.25).item() if valid_count > 0 else 0.0,
        "pg_update/clip/high/count": float(clip_high_count),
        "pg_update/clip/low/count": float(clip_low_count),
        "pg_update/clip/dual/count": float(dual_clip_count),
        "pg_update/clip/total/count": float(clip_total_count),
        "pg_update/clip/high/rate": (clip_high_count / valid_count) if valid_count > 0 else 0.0,
        "pg_update/clip/low/rate": (clip_low_count / valid_count) if valid_count > 0 else 0.0,
        "pg_update/clip/dual/rate": (dual_clip_count / valid_count) if valid_count > 0 else 0.0,
        "pg_update/clip/total/rate": (clip_total_count / valid_count) if valid_count > 0 else 0.0,
        "pg_update/clip/valid_token_count": float(valid_count),
        "pg_update/clip/neg_adv_token_count": float((adv_valid < 0).sum().item()) if valid_count > 0 else 0.0,
    }
    return pg_loss, pg_metrics


@register_policy_loss("cum-token-geomean")
def compute_policy_loss_cum_token_geomean(
    old_log_prob: torch.Tensor,
    log_prob: torch.Tensor,
    advantages: torch.Tensor,
    response_mask: torch.Tensor,
    ...
    return _compute_policy_loss_cum_token_impl(
...
@register_policy_loss("cum-token-cumprod")
def compute_policy_loss_cum_token_cumprod(
    old_log_prob: torch.Tensor,
    log_prob: torch.Tensor,
    advantages: torch.Tensor,
    response_mask: torch.Tensor,
...
# ---------------------------------------------------------------------------
# Length-Adaptive CTPO (CTPO-LA)
# ---------------------------------------------------------------------------
# All three clip parameters (la_clip_high, la_clip_low, la_clip_c) are
# treated as **log-space base values** that scale with absolute position t:
#
#   adaptive_log_clip_high(t) = la_clip_high · t^la_power
#   adaptive_log_clip_low(t)  = la_clip_low  · t^la_power
#   adaptive_log_clip_c(t)    = la_clip_c    · t^la_power
#
# Effective ratio window at position t:
#   [exp(-la_clip_low · t^p),  exp(la_clip_high · t^p)]
#
# Dual-clip ratio threshold:
#   exp(la_clip_c · t^p)
#
# At t=1 the window is [exp(-la_clip_low), exp(la_clip_high)], which with
# small base values (e.g. 0.025 / 0.05) is nearly [0.975, 1.05] — tight but
# not degenerate.  The window then grows gradually with position, matching
# the variance growth of the cumulative log-ratio.
#
# With la_power=0, scale=1 for all positions (uniform clip).
#
# Default base values (match ctpo_la*.sh): la_clip_low=0.025, la_clip_high=0.05, la_clip_c=0.05
#
# Recommended:
#   cumprod → la_power = +0.5  (Var(log_r_t) ∝ t)
#   geomean → la_power = -0.5  (Var(log_r_t) ∝ 1/t)
# ---------------------------------------------------------------------------


def _compute_policy_loss_cum_token_la_impl(
    old_log_prob: torch.Tensor,
    log_prob: torch.Tensor,
    advantages: torch.Tensor,
    response_mask: torch.Tensor,
    ...
    config: Optional[ActorConfig] = None,
    rollout_is_weights: torch.Tensor | None = None,
    ratio_mode: str = "cumprod",
) -> tuple[torch.Tensor, dict[str, Any]]:
    """Length-adaptive CTPO loss with log-space clipping that scales with position.

    ``la_clip_high``, ``la_clip_low``, and ``la_clip_c`` are **log-space**
    base values (e.g. 0.05 / 0.03 / 0.05).  At token position *t* they are
    multiplied by ``t ** la_power``:

    * t=1: clip window ≈ [exp(-la_clip_low), exp(la_clip_high)] — tight
    * t grows: window widens in proportion to cumulative ratio variance
    * la_power=0: uniform clip identical to original CTPO-style loss

    Configured via ``actor.la_clip_high``, ``actor.la_clip_low``,
    ``actor.la_clip_c``, and ``actor.clip_ratio_la_power``.
    """

    assert config is not None
    assert isinstance(config, ActorConfig)
    # la_clip_* are LOG-SPACE base values, not ratio-space.
    la_clip_low:  float = config.get("la_clip_low",  0.025)
    la_clip_high: float = config.get("la_clip_high", 0.05)
    la_clip_c:    float = config.get("la_clip_c",    0.05)
    la_power:     float = config.get("clip_ratio_la_power", 0.5)

    assert la_clip_low  > 0, f"la_clip_low must be positive, got {la_clip_low}"
    assert la_clip_high > 0, f"la_clip_high must be positive, got {la_clip_high}"
    assert la_clip_c    > 0, f"la_clip_c must be positive, got {la_clip_c}"

    # ---- cumulative importance ratio (same as original CTPO) ---------------
    negative_approx_kl = log_prob - old_log_prob
    negative_approx_kl = torch.clamp(negative_approx_kl, min=-20.0, max=20.0)
    ...
    # Stop-gradient decomposition (same as original CTPO).
    log_importance_ratio = kl_values.detach() + log_prob - log_prob.detach()
    log_importance_ratio = torch.clamp(log_importance_ratio, max=10.0)
    ratio = torch.where(mask > 0, torch.exp(log_importance_ratio), torch.zeros_like(log_importance_ratio))
    ...
    # ---- absolute 1-indexed position within response -----------------------
    pos_t = torch.cumsum(mask, dim=-1)  # [batch, seq_len], 0 for padding

    # ---- position-scaled log-space clip bounds ----------------------------
    if la_power != 0.0:
        scale = pos_t.clamp(min=1.0).pow(la_power)        # [batch, seq_len]
        adaptive_log_clip_low  = la_clip_low  * scale
        adaptive_log_clip_high = la_clip_high * scale
        adaptive_log_clip_c    = la_clip_c    * scale
    else:
        adaptive_log_clip_low  = la_clip_low
        adaptive_log_clip_high = la_clip_high
        adaptive_log_clip_c    = la_clip_c

    # ---- clipped ratio (log-space clamp → ratio always > 0) ---------------
    log_r_clipped = torch.clamp(log_importance_ratio, -adaptive_log_clip_low, adaptive_log_clip_high)
    clipped_ratio = torch.where(mask > 0, torch.exp(log_r_clipped), torch.zeros_like(log_r_clipped))

    # Dual-clip threshold grows with position: exp(la_clip_c · t^p)
    dual_clip_threshold = torch.where(
        mask > 0, torch.exp(adaptive_log_clip_c), torch.ones_like(adaptive_log_clip_c)
    )
    ...
    pg_losses3 = -advantages * dual_clip_threshold
    clip_pg_losses2 = torch.min(pg_losses3, clip_pg_losses1)
    pg_clipfrac_lower = verl_F.masked_mean(
        torch.gt(clip_pg_losses1, pg_losses3) * (advantages < 0).float(), response_mask
    )
    ...
    log_r_valid = log_importance_ratio[valid_mask]
    if la_power != 0.0:
        log_clip_high_valid = adaptive_log_clip_high[valid_mask]
        log_clip_low_valid  = adaptive_log_clip_low[valid_mask]
    else:
        log_clip_high_valid = torch.full_like(log_r_valid, la_clip_high)
        log_clip_low_valid  = torch.full_like(log_r_valid, la_clip_low)

    adv_valid_full = advantages[valid_mask]
    clip_high_mask_v = (log_r_valid >  log_clip_high_valid) & (adv_valid_full > 0)
    clip_low_mask_v  = (log_r_valid < -log_clip_low_valid)  & (adv_valid_full < 0)
    dual_clip_mask   = torch.gt(clip_pg_losses1, pg_losses3) & (advantages < 0) & valid_mask
    dual_clip_mask_v = dual_clip_mask[valid_mask]

    clip_high_count  = int(clip_high_mask_v.sum().item())
    clip_low_count   = int(clip_low_mask_v.sum().item())
    dual_clip_count  = int(dual_clip_mask_v.sum().item())
    clip_total_count = int((clip_high_mask_v | clip_low_mask_v | dual_clip_mask_v).sum().item())
    ...
    if la_power != 0.0 and valid_count > 0:
        scale_valid = scale[valid_mask]
        pg_metrics.update({
            "pg_update/la_scale/mean": scale_valid.mean().item(),
            "pg_update/la_scale/min": scale_valid.min().item(),
            "pg_update/la_scale/max": scale_valid.max().item(),
            "pg_update/la_scale/p50": torch.quantile(scale_valid, 0.50).item(),
            "pg_update/la_clip_high_ratio/mean": torch.exp(la_clip_high * scale_valid).mean().item(),
            "pg_update/la_clip_low_ratio/mean":  torch.exp(-la_clip_low * scale_valid).mean().item(),
            "pg_update/la_clip_c_ratio/mean":    torch.exp(la_clip_c * scale_valid).mean().item(),
        })

    return pg_loss, pg_metrics


@register_policy_loss("cum-token-cumprod-la")
def compute_policy_loss_cum_token_cumprod_la(
    old_log_prob: torch.Tensor,
    log_prob: torch.Tensor,
    advantages: torch.Tensor,
    response_mask: torch.Tensor,
    ...
    return _compute_policy_loss_cum_token_la_impl(
        old_log_prob=old_log_prob,
        log_prob=log_prob,
        advantages=advantages,
        response_mask=response_mask,
...
@register_policy_loss("cum-token-geomean-la")
def compute_policy_loss_cum_token_geomean_la(
    old_log_prob: torch.Tensor,
    log_prob: torch.Tensor,
    advantages: torch.Tensor,
    response_mask: torch.Tensor,
