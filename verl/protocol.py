...
def list_of_dict_to_dict_of_list(
    ...
    *,
    ...
    # Robust mode used for metric aggregation:
    # align keys across workers and fill missing entries with a default scalar.
    keys: set[Any] = set()
    for data in list_of_dict:
    ...
    for data in list_of_dict:
        for key in keys:
            output[key].append(data.get(key, missing_value))
    return output
...
class DataProto:
    ...
    def concat(data: list["DataProto"]) -> "DataProto":
        ...
        if data:
            ...
            if all_metrics:
                merged_meta_info["metrics"] = list_of_dict_to_dict_of_list(
                    all_metrics, allow_missing_keys=True, missing_value=0.0
                )
