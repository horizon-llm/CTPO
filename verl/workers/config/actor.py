...
class ActorConfig(BaseConfig):
    ...
    clip_ratio_la_power: float = 0.0
    # CTPO-LA log-space clip base values (used only by cum-token-*-la losses).
    # These are log-space values that scale with position t^la_power.
    # Effective ratio window at position t: [exp(-la_clip_low·t^p), exp(la_clip_high·t^p)]
    # Default matches ctpo_la*.sh: la_clip_low=0.025, la_clip_high=0.05, la_clip_c=0.05
    la_clip_low: float = 0.025
    la_clip_high: float = 0.05
    la_clip_c: float = 0.05
