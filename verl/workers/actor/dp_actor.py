...
class DataParallelPPOActor(BasePPOActor):
    ...
    def update_policy(self, data: DataProto):
        ...
        metrics = {
            "actor/pg_loss": 0.0,
            "actor/kl_loss": 0.0,
            "actor/debug/num_minibatches": float(len(mini_batches)),
            "actor/debug/on_policy_flag": 1.0 if on_policy else 0.0,
        }
        for _ in range(self.config.ppo_epochs):
            for batch_idx, mini_batch in enumerate(mini_batches):
                is_last_minibatch = batch_idx == (len(mini_batches) - 1)
                last_minibatch_ratio_chunks: list[torch.Tensor] = []
                track_last_minibatch_ratio = False
                ...
                for micro_batch in micro_batches:
                    micro_batch = micro_batch.to(get_device_id())
                    # Keep metric keys stable across workers/micro-batches to avoid
                    # DataProto.concat failures when one rank has empty response masks.
                    micro_batch_metrics = {
                        "actor/empty_response_mask": 0.0,
                        "actor/non_finite_pg_loss": 0.0,
                    }
                    model_inputs = {**micro_batch.batch, **micro_batch.non_tensor_batch, "pad_token_id": pad_token_id}
                    response_mask = model_inputs["response_mask"]
                    # Record empty-mask micro-batches but do not branch out; keep all ranks on the same path.
                    has_valid_response = torch.any(response_mask)
                    if not has_valid_response:
                        micro_batch_metrics["actor/empty_response_mask"] = 1.0
                    ...
                    track_last_minibatch_ratio = loss_mode in {"cum-token-geomean", "cum-token-cumprod"}
                    ...
                    if not torch.isfinite(pg_loss):
                        micro_batch_metrics["actor/non_finite_pg_loss"] = 1.0
                    micro_batch_metrics.update(pg_metrics)

                    # Debug: track the IS-ratio distribution of the LAST mini-batch only.
                    # This helps verify whether multiple updates move ratios away from 1.
                    if is_last_minibatch and track_last_minibatch_ratio and has_valid_response:
                        with torch.no_grad():
                            valid_mask = response_mask > 0
                            ratio_for_debug = None
                            mask_float = response_mask.to(dtype=log_prob.dtype)
                            delta = torch.clamp(log_prob - old_log_prob, min=-20.0, max=20.0)

                            if loss_mode in {"cum-token-geomean", "cum-token-cumprod"}:
                                cumulative_sum = torch.cumsum(delta * mask_float, dim=-1)
                                if loss_mode == "cum-token-geomean":
                                    cumulative_count = torch.cumsum(mask_float, dim=-1).clamp(min=1.0)
                                    kl_values = torch.where(
                                        mask_float > 0, cumulative_sum / cumulative_count, torch.zeros_like(cumulative_sum)
                                    )
                                else:
                                    kl_values = torch.where(mask_float > 0, cumulative_sum, torch.zeros_like(cumulative_sum))
                                ...
                                ratio_for_debug = torch.where(
                                    mask_float > 0, torch.exp(log_importance_ratio), torch.zeros_like(log_importance_ratio)
                                )

                            if ratio_for_debug is not None:
                                ratio_valid = ratio_for_debug[valid_mask]
                                if ratio_valid.numel() > 0:
                                    last_minibatch_ratio_chunks.append(ratio_valid.detach())

                    # Skip if using bypass_mode loss (metrics already computed in pg_metrics)
                    rollout_log_prob = model_inputs.get("rollout_log_probs", None)
                    if loss_mode != "bypass_mode" and rollout_log_prob is not None:
                        # Always emit the same rollout_corr metric keys. If this micro-batch has
                        # no valid response tokens, keep zero defaults to preserve key alignment.
                        rollout_corr_metrics = {
                            "rollout_corr/training_ppl": 0.0,
                            "rollout_corr/training_log_ppl": 0.0,
                            "rollout_corr/kl": 0.0,
                            "rollout_corr/k3_kl": 0.0,
                            "rollout_corr/rollout_ppl": 0.0,
                ...
                if is_last_minibatch and track_last_minibatch_ratio:
                    if len(last_minibatch_ratio_chunks) > 0:
                        ratio_all = torch.cat(last_minibatch_ratio_chunks, dim=0)
                        valid_n = float(ratio_all.numel())
                        mini_batch_metrics.update(
                            {
                                "actor/debug/last_minibatch/is_ratio/mean": ratio_all.mean().item(),
                                "actor/debug/last_minibatch/is_ratio/min": ratio_all.min().item(),
                                "actor/debug/last_minibatch/is_ratio/max": ratio_all.max().item(),
                                "actor/debug/last_minibatch/is_ratio/p99": torch.quantile(ratio_all, 0.99).item(),
                                "actor/debug/last_minibatch/is_ratio/p95": torch.quantile(ratio_all, 0.95).item(),
                                "actor/debug/last_minibatch/is_ratio/p90": torch.quantile(ratio_all, 0.90).item(),
                                "actor/debug/last_minibatch/is_ratio/p75": torch.quantile(ratio_all, 0.75).item(),
                                "actor/debug/last_minibatch/is_ratio/p50": torch.quantile(ratio_all, 0.50).item(),
                                "actor/debug/last_minibatch/is_ratio/p25": torch.quantile(ratio_all, 0.25).item(),
                                "actor/debug/last_minibatch/is_ratio/valid_token_count": valid_n,
                            }
                        )
                    else:
                        mini_batch_metrics.update(
                            {
                                "actor/debug/last_minibatch/is_ratio/mean": 0.0,
                                "actor/debug/last_minibatch/is_ratio/min": 0.0,
                                "actor/debug/last_minibatch/is_ratio/max": 0.0,
                                "actor/debug/last_minibatch/is_ratio/p99": 0.0,
                                "actor/debug/last_minibatch/is_ratio/p95": 0.0,
                                "actor/debug/last_minibatch/is_ratio/p90": 0.0,
                                "actor/debug/last_minibatch/is_ratio/p75": 0.0,
                                "actor/debug/last_minibatch/is_ratio/p50": 0.0,
                                "actor/debug/last_minibatch/is_ratio/p25": 0.0,
                                "actor/debug/last_minibatch/is_ratio/valid_token_count": 0.0,
                            }
                        )
